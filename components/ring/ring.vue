...
export default {
  methods: {
    setClip(t, r, b, l) {
      return `rect(${t}rpx, ${r}rpx, ${b}rpx, ${l}rpx)`
    },
  ...
  data() {
    ...
    return {
      ...
      circleStyle: {
        ...
        borderStyle: 'solid',
        borderWidth: DEFAULT_BOLD + 'rpx',
        width: (this.width || DEFAULT_WIDTH) + 'rpx',
        height: (this.width || DEFAULT_WIDTH) + 'rpx',
        backgroundColor: (this.bgColor || DEFAUTL_BG_COLOR)
      },
      // 进度文字样式
      progressTextStyle: {
        fontSize: (this.fontSize || DEFAULT_FONT_SIZE) + 'rpx',
